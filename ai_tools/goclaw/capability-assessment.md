...
> 核心问题：能否给一句话需求或简单的需求话术文件，自动做出项目？
...
4. **生成代码** —— 同场景一的生成能力
5. **表格解析** —— 能识别简单的需求话术文件中的表格、条款、附件

### 做不到的 ❌
1. **无标准化需求提取** —— 解析结果只传给 LLM，没有持久化为结构化需求单
2. **无公文格式理解的专用工具** —— 没有针对公文的 prompt 模板或 SKILL.md
...
> **上传简单的需求话术文件 → 提取需求 → 生成 PRD → 脚手架初始化 → 逐模块开发（多 Agent 并行）→ 自动测试修复 → 输出完整项目**
